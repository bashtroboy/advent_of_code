transmission = DATA.read    # Hex transmission as its received
@bits = ""                   # the transmission converted to binary
@versions = []               # record all @versions in transmission

# convert Hex transmission to binary
transmission.each_char do |ch|
    # convert hex to binary, and ensure digits are four in length with padding
    @bits += ch.to_i(16).to_s(2).rjust(4,"0")
end

def parse_sub_packet(bitz)
    puts bitz
    version = 0
    type = 0

    # find out the version and type for this sub-packet
    version = bitz[0..2].to_i(2)
    type = bitz[3..5].to_i(2)

    # add the version to the @versions array
    @versions << version
    puts "version=#{version}, type=#{type}"
    # trim the version and type @bits
    bitz.slice!(0..5)
    if type == 4  # literal sub-packet
        last_char = false
        literal = ""
        
        while last_char == false do
            bitz[0] == "0" ? last_char = true : nil
            literal += bitz[1..4]
            bitz.slice!(0..4)
        end
    else # operator sub-packet
        length_type = bitz[0] # the first character after the header is the length indicator, one digit
        bitz.slice!(0)

        if length_type == "0" # the next 15 @bits is a number that represents the total length in @bits of the contained sub-packets
            
            total_length = bitz[0..14].to_i(2)
            bitz.slice!(0..14)

            sub_packet = bitz[0..(total_length-1)]
            bitz.slice!(0..(total_length-1))
            parse_sub_packet(sub_packet)
        else # type is 1, the next 11 @bits represent the number of sub-packets contained in the packet
            
            num_sub_pack = bitz[0..10].to_i(2)
            bitz.slice!(0..10)

            sub_packet = bitz[0..((11*num_sub_pack)-1)]
            bitz.slice!(0..((11*num_sub_pack)-1))
            while sub_packet.length > 0 do
                parse_sub_packet(sub_packet[0..10])
                sub_packet.slice!(0..10)
            end
        end

        # trim remaining zeroes
        while bitz[0] == "0" do
            bitz.slice!(0)
        end
    end
end

while @bits.length > 0
    version = 0
    type = 0

    # find out the version and type for this sub-packet
    version = @bits[0..2].to_i(2)
    type = @bits[3..5].to_i(2)

    # add the version to the @versions array
    @versions << version
    puts "version=#{version}, type=#{type}"
    # trim the version and type @bits
    @bits.slice!(0..5)
    if type == 4  # literal sub-packet
        last_char = false
        literal = ""
        
        while last_char == false do
            @bits[0] == "0" ? last_char = true : nil
            literal += @bits[1..4]
            @bits.slice!(0..4)
        end

        # trim remaining zeroes
        while @bits[0] == "0" do
            @bits.slice!(0)
        end

    else # operator sub-packet
        length_type = @bits[0] # the first character after the header is the length indicator, one digit
        @bits.slice!(0)

        if length_type == "0" # the next 15 @bits is a number that represents the total length in @bits of the contained sub-packets
            
            p total_length = @bits[0..14].to_i(2)
            @bits.slice!(0..14)

            sub_packet = @bits[0..(total_length-1)]
            @bits.slice!(0..(total_length-1))
            parse_sub_packet(sub_packet)
        else # type is 1, the next 11 @bits represent the number of sub-packets contained in the packet
            
            num_sub_pack = @bits[0..10].to_i(2)
            @bits.slice!(0..10)

            sub_packet = @bits[0..((11*num_sub_pack)-1)]
            @bits.slice!(0..((11*num_sub_pack)-1))
            while sub_packet.length > 0 do
                parse_sub_packet(sub_packet[0..10])
                sub_packet.slice!(0..10)
            end
        end

        # trim remaining zeroes
        while @bits[0] == "0" do
            @bits.slice!(0)
        end
    end
end


puts
p @versions

=begin

- First three @bits are the packet version number

- Second three @bits are the packet type ID
    - Type 4: Literal number.  Subesequent sets of 5 @bits start with a 1 if not the last character, and 0 if the last character
        The folowing four @bits are the @bits to make up the literal number

=end
    
# #
__END__
8A004A801A8002F478